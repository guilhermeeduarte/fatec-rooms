...
// Função para verificar se um período é exclusivo de sábado (baseado no nome)
function isSaturdayOnlyPeriod(period) {
  if (!period.name) return false;
  const nomeLower = period.name.toLowerCase();
  return nomeLower.includes("sábado") || nomeLower.includes("sab");
}
...
export default function ReservaRecorrente() {
  ...
  const filteredPeriods = useMemo(() => {
    ...
    return activePeriods.filter((period) => {
      ...
      // Se é período exclusivo de sábado, só mostra se sábado está selecionado
      if (isSaturdayOnly) return hasSaturday;
      // Períodos normais (não sábado) são sempre exibidos
      return true;
    });
  }, [activePeriods, form.weekDays]);

  // Efeito para remover períodos de sábado da seleção quando o sábado for desmarcado
  useEffect(() => {
    const hasSaturday = form.weekDays.includes("SAB");
  ...
  // Set de ISOs marcados (dias da semana selecionados dentro do semestre)
  ...
  function handleChange(e) {
    ...
    setForm((prev) => {
      ...
      if (name === "courseId") next.classGroupId = "";
      if (name === "semesterId") { next.weekDays = []; next.periodIds = []; }
      return next;
    });
    setSuccess(null);
    setError(null);
  ...
  function toggleDia(code) {
    if (!code) return;
    if (code === "SAB" && selectedClassGroup && !selectedClassGroup.hasSaturday) return;
    setForm((prev) => ({
      ...prev,
      weekDays: prev.weekDays.includes(code)
  ...
  function togglePeriod(periodId) {
    setForm((prev) => ({
      ...
      periodIds: prev.periodIds.includes(periodId)
        ? prev.periodIds.filter((id) => id !== periodId)
            ...
            <div className="form-group-reserva">
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "0.25rem" }}>
                {dayCodeLabels.map((dia) => {
                  const disabled = dia === "SAB" && selectedClassGroup && !selectedClassGroup.hasSaturday;
            ...
            <div className="form-group-reserva">
              ...
              <div className="period-dropdown-options" style={{ position: "static", boxShadow: "none", border: "1px solid #e5e7eb", borderRadius: "8px" }}>
                <div style={{ marginBottom: 8, display: "flex", gap: 8 }}>
                  <button type="button" style={{ fontSize: 12, padding: "2px 8px", borderRadius: 6, border: "1px solid #ccc", cursor: "pointer", background: "white" }} onClick={() => setForm(prev => ({ ...prev, periodIds: filteredPeriods.map(p => p.id) }))}>Selecionar todos</button>
                  <button type="button" style={{ fontSize: 12, padding: "2px 8px", borderRadius: 6, border: "1px solid #ccc", cursor: "pointer", background: "white" }} onClick={() => setForm(prev => ({ ...prev, periodIds: [] }))}>Limpar</button>
                </div>
                {filteredPeriods.length === 0 && (
                  <div style={{ padding: "0.5rem", textAlign: "center", color: "#888" }}>
                    {!form.weekDays.includes("SAB") 
                      ? "Nenhum período disponível (os períodos de sábado só aparecem se você selecionar Sábado)." 
                ...
                {filteredPeriods.map((period) => (
                  <label key={period.id} className="period-checkbox">
                    <input
                      type="checkbox"
                      checked={form.periodIds.includes(period.id)}
